...
Quick Search (v3)
====
Search component written in HTL. Extends the Quick Search (v2) component, adding an opt-in
AI Search toggle.
...
inherited unchanged from Quick Search (v2).

### AI Search Toggle
The component renders an additional toggle next to the search input. When enabled by the visitor, the
`?{}?` prefix is prepended to the `fulltext` search parameter, routing the query to ContentAI-powered
semantic search instead of the default JCR/QueryBuilder full-text search. The prefix is only added
client-side; `SearchResultServlet` and the result item markup are unchanged from v2.

### Behavior
When the user is scrolling down the results, if the hidden results below are less than the visible results,
