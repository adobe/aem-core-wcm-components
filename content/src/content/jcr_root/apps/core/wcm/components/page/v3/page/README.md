...
1`./clientlibsJsHead` - allows definition of client libraries for which JavaScript is specifically intended to be loaded
in the document head (JavaScript only) of pages associated with this policy configuration
1. `./isCustomJsClientlibsAsync` - allows custom Javascript libraries to be loaded asynchronously
1`./appResourcesClientlib` - allows definition of the client library that is used to serve web resources such as favicons
1`./renderAlternateLanguageLinks` - toggles on/off the rendering of links to alternate language versions of the page in its head
