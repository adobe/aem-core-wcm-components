...
/*
    @Test
    @DisplayName("Test: Nested")
    public void AllowedComponents() throws TimeoutException, InterruptedException, ClientException {

        String policyName = "/accordion";
        String policyLocation = "core-component/components";
        String pageRT = "core/wcm/tests/components/test-page-v2";

        String teaserProxyPath = Commons.createProxyComponent(adminClient, rtTeaser_v1, Commons.proxyPath, null, null);
        ...
        String policyPath1 = "/conf/"+ label + "/settings/wcm/policies/core-component/components";
        String policyPath = Commons.createPolicy(adminClient, policyName + "/new_policy", data , policyPath1, 201);

        data.clear();
        data.put("cq:policy", policyLocation + policyName + "/new_policy");
        data.put("sling:resourceType", "wcm/core/components/policies/mappings");
        String policyAssignmentPath = defaultPageTemplate + "/policies/jcr:content";// + "/root/responsivegrid";
        Commons.assignPolicy(adminClient, policyName, data, policyAssignmentPath, 201);

        String proxyPath = Commons.createProxyComponent(adminClient, rtAccordion_v1, Commons.proxyPath, null, null);

        data.clear();
        data.put("cq:isContainer","true");
        Commons.editNodeProperties(adminClient, proxyPath, data);

        String testPage = Commons.createPage(adminClient,"testPage", "Test Page Title", rootPage, defaultPageTemplate,"core/wcm/tests/components/test-page-v2");

        String cmpPath = Commons.addComponent(adminClient, proxyPath,testPage + Commons.relParentCompPath, null, null);

        PageEditorPage editorPage = new PageEditorPage(testPage);
        editorPage.open();
        //accordion.getCQOverlay().openComponentOverlay(cmpPath + "/*");

        EditableToolbar editableToolbar = editorPage.openEditableToolbar(cmpPath + "/*");
        EditableToolbarAssertion editableToolbarAssertion = new EditableToolbarAssertion(editableToolbar,
            "editable toolbar of none style selector enabled component - %s button is displayed while it should not");
        editableToolbarAssertion.assertInsertButton(true);
        editableToolbar.clickInsertComponent();

        $("coral-selectlist-item[value='"+teaserProxyPath+"']").should(Condition.visible);

        Commons.deleteProxyComponent(adminClient, teaserProxyPath);

        authorClient.deletePageWithRetry(testPage, true,false, WCMSanityConstants.TIMEOUT_TIME_MS, WCMSanityConstants.RETRY_TIME_INTERVAL,  HttpStatus.SC_OK);

        policyPath1 = "/conf/"+ label + "/settings/wcm/policies/core-component/components";
        Commons.deletePolicy(adminClient,"/structure/page", policyPath1);
