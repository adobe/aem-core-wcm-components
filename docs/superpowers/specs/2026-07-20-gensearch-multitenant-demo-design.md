...
2. **Minting endpoint** (new service): validates the incoming Bearer token, decodes `projectedProductContext`/`roles`, mints the short-lived signed handoff token, and forwards a scoped user credential alongside it (§5.2 step 6, §7).
