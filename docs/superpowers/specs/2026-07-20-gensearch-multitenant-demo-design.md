...
3. **The minting endpoint** validates the token's signature and decodes `projectedProductContext` (does this user have entitlement to this specific environment/bucket?) and `roles` (do they have `dx_aem_b_env_user` or `dx_aem_b_administrator`?). If valid, it mints a **short-lived (60–120s), single-use, bucket-scoped token**, signed with our own secret (HMAC/JWT) — unrelated to AEM identity, IMS, or any AEM/JCR session.
...
6. **The demo page itself** — same gensearch component family as WKND — calls Content AI's search/gensearch endpoints using **anonymous `x-api-key`** (Option B), exactly like the existing WKND setup: a per-org API key configured at onboarding time (OSGi config, same shape as `ContentAIClientImpl`'s existing `defaultContentSource`/`baseUrlOverride` pattern), assuming that org's content source is marked public. This is deliberately decoupled from the viewing-gate in step 3–5: "who can reach the page" and "how the page talks to Content AI" are two independent concerns, and the latter reuses a mechanism already proven stable all session, sidestepping gensearch's unfinished S2S story entirely.
