...
**Date:** 2026-07-20
**Status:** Draft — pending user review and hosting-model decision (see §9, §10)
...
6. **The demo page itself** — same gensearch component family as WKND — calls Content AI's search/gensearch endpoints using the **forwarded user Bearer token, uniformly, for every content source regardless of its public/private status.** This superseded an earlier draft of this design that branched on public-vs-private status (anonymous `x-api-key` for public sources, Bearer only for private ones) - dropped for a concrete, well-grounded reason, not just simplicity for its own sake:
   ...
   **Mechanically**: the minting endpoint forwards the original IMS user token itself (or a narrowly-scoped exchange token representing the same user, valid only for Content AI calls) alongside the handoff token from step 3, and the demo page attaches it directly to its own Content AI calls for exactly as long as it remains valid - no refresh, no extension. Once the original token expires, search simply stops working until the user clicks through from Cloud Manager again for a fresh one - consistent with this being a demo flow gated to authenticated CM sessions (§2), not a permanently-public page. The forwarded credential gets the same handling as the handoff token itself (§7): never logged, never cached, HTTPS only, discarded rather than persisted server-side.
...
- **Forwarding a live, user-scoped IMS credential to the destination page is materially more sensitive than a one-time verification**, and now applies to every content source, not just a subset. It gets the exact same handling as the handoff token (never logged, never cached, HTTPS only, discarded not persisted), but the blast radius of getting this wrong is real - this path needs its own explicit security review before implementation.
- **Empirically verified, not just documented**: a real captured CM Bearer token (`client_id: ssp_ui`, scope including `contentai.api`) was tested directly against both live endpoints on `publish-p149667-e1527800.adobeaemcloud.com` - `POST /content-sources/search` (HTTP 200, real ranked results for content source `wknd`) and `POST /content-sources/gensearch` (HTTP 200, a real generated answer with source citations). This confirms the private-source design's core assumption - that the same user Bearer token CM already holds genuinely authenticates against both APIs the demo page needs - end to end, not just per the documented "Option A" listing.
...
2. **Minting endpoint** (new Adobe I/O Runtime action, decoupled from the hosting-model decision in §10 — see §5.2 step 3): validates the incoming Bearer token, decodes `projectedProductContext`/`roles`, mints the short-lived signed handoff token, and forwards a scoped user credential alongside it (§5.2 step 6, §7).
3. **Destination demo page + servlet** (per org, hosting model TBD — see §9/§10): redemption/session logic, plus the existing gensearch component family (reused from `aem-core-wcm-components`, WKND-style), configured per org, calling Content AI with the forwarded Bearer token.
4. **`ContentAIClientImpl` Bearer-token support** (`aem-core-wcm-components` code change, confirmed necessary in §5.2 step 6, and confirmed working end-to-end against the live API): today this component only ever sends `X-Api-Key`; add a code path to call Content AI with `Authorization: Bearer <token>` instead. One small, additive change, needed regardless of which way the hosting-model decision (§10) goes - not duplicated per option.
